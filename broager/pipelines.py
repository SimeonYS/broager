...
class BroagerPipeline:

    # Database setup
    conn = sqlite3.connect('broager.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `broager`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM broager WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO broager (date, title, link, content)"
